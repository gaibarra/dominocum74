...
export const getGameIdForTable = async (client, tableId) => {
  const sql = 'SELECT game_id FROM game_tables WHERE id = $1';
...
export const createOrGetOpenPartidaForTable = async (client, tableId) => {
  ...
  const gameId = await getGameIdForTable(trx, tableId);
