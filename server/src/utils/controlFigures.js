import { computePlayingMinutes } from './attendance.js';
...
const isCancelledTable = (table) => Boolean(table?.cancelled_at || table?.canceled_at || table?.cancelledAt);
...
export const buildGameControlFigures = ({ game = {}, attendance = [] } = {}) => {
  ...
  const finishedTables = tables.filter((table) => Boolean(table?.partidaFinished ?? table?.partida_finished)).length;
  const cancelledTables = tables.filter((table) => isCancelledTable(table)).length;
  const activeTables = tables.filter((table) => {
    const isFinished = table?.partidaFinished ?? table?.partida_finished;
    const cancelled = isCancelledTable(table);
    const hasHands = Array.isArray(table?.hands) && table.hands.length > 0;
    return !isFinished && !cancelled && hasHands;
  }).length;

  let totalHands = 0;
  let totalPoints = 0;
  let partidasRegistradas = 0;
  let partidasEnCurso = 0;
  ...
  tables.forEach((table) => {
    ...
    const terminadas = numberOrZero(table?.games_won_pair1) + numberOrZero(table?.games_won_pair2);
    const hayPartidaEnCurso = !Boolean(table?.partidaFinished ?? table?.partida_finished) && !isCancelledTable(table) && hands.length > 0 ? 1 : 0;
    partidasRegistradas += terminadas;
    partidasEnCurso += hayPartidaEnCurso;
  ...
  const partidasTotales = partidasRegistradas + partidasEnCurso;
...
export const buildStatsControlFigures = ({ totalGames = 0, totalHands = 0, players = [] } = {}) => {
