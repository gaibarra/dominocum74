...
const addTransformIndexHtml = (enableScripts) => ({
	name: 'add-transform-index-html',
	transformIndexHtml(html) {
		if (!enableScripts) return { html, tags: [] };
		return {
			html,
			tags: [
...
export default defineConfig(({ mode }) => {
	const enableHorizonsScripts = mode === 'development' && process.env.VITE_HORIZONS === '1';
	return {
		customLogger: logger,
		plugins: [react(), addTransformIndexHtml(enableHorizonsScripts)],
		server: {
			cors: true,
			headers: {
